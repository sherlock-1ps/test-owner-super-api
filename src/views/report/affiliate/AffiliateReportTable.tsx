...
const AffiliateReportTable = ({ activeCard }: { activeCard: number }) => {
  // States
  const [status, setStatus] = useState<InvoiceType['invoiceStatus']>('')
  const [rowSelection, setRowSelection] = useState({})
  const [data, setData] = useState<InvoiceTypeWithAction[]>([])
  ...
  const columns = useMemo<ColumnDef<InvoiceTypeWithAction, any>[]>(
    () => [
      columnHelper.accessor('id', {
        header: 'UserID',
        cell: ({ row }) => <Typography variant='h6'>{`XPB#${row.original.id}`}</Typography>
      }),

      columnHelper.accessor('group', {
        header: () => <div className='w-full text-center'>Group</div>,
          ...
          <div className='flex items-center gap-3'>
            <div className='flex flex-col w-full'>
              {row.original.group && (
                <Chip
                  variant='tonal'
                  label={getGroupLabelPlayer(row.original.group)}
                  size='small'
                  sx={{
                    background: getGroupPlayerGradient(row.original.group)
                  }}
      ...
      columnHelper.accessor('balance', {
        ...
        cell: ({ row }) => <Typography className='text-center'>{`${row.original.group}`}</Typography>
      }),
      columnHelper.accessor('issuedDate', {
        header: 'Affiliate Total bet',
        cell: ({ row }) => <Typography color={'primary.main'}>$ {row.original.issuedDate}</Typography>
      }),
      columnHelper.accessor('cashback', {
        header: 'Affiliate Commission',
        cell: ({ row }) => {
          return <Typography color={'primary.main'}>+$ {row.original.cashback}</Typography>
        }
      })
    ],
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    let selectedData: InvoiceTypeWithAction[] = []

    if (activeCard === 1) {
      selectedData = dataMock || []
    } else if (activeCard === 2) {
      selectedData = dataMock2 || []
    } else if (activeCard === 3) {
      selectedData = dataMock3 || []
    } else if (activeCard === 4) {
      selectedData = dataMock || []
    }

    const sortedData = [...selectedData].sort((a, b) => (a.group ?? 0) - (b.group ?? 0))
...
export default AffiliateReportTable
