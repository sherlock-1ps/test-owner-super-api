...
const dataMock = [
  ...
  },
  {
    ...
    textPromotion: '🎮สมาชิกใหม่ฝาก 50 รับ 100 บาท'
  },
  {
    id: '4989',
    issuedDate: 793,
...
const PromotionReportTable = () => {
  ...
  const columns = useMemo<ColumnDef<InvoiceTypeWithAction, any>[]>(
    () => [
      columnHelper.accessor('textPromotion', {
        header: 'Promotions',

        cell: ({ row }) => <Typography variant='h6'> {row.original.textPromotion}</Typography>
      }),
      ...
      columnHelper.accessor('issuedDate', {
        ...
        cell: ({ row }) => <Typography color={'success.main'}>$ {row.original.issuedDate}</Typography>
      }),
      columnHelper.accessor('bonus', {
        header: 'Amount Bonus',
        cell: ({ row }) => <Typography color={'info.main'}>{`$ ${row.original.bonus}`}</Typography>
      })
    ],
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [data, filteredData]
...
export default PromotionReportTable
