...
const BankAccountReportCardSummary = (props: CardStatsHorizontalWithBorderProps) => {
  // Props
  const { title, stats, avatarIcon, color } = props

  return (
    <Card color={color || 'primary'} className='min-h-[140px] flex items-center'>
      <CardContent className='flex flex-col gap-1 justify-center'>
        <div className='flex items-center gap-4'>
          ...
          <Typography variant='h4'>
            {color == 'primary' ? '' : '$'}
            {new Intl.NumberFormat().format(Number(stats))}
          </Typography>
        </div>
...
export default BankAccountReportCardSummary
