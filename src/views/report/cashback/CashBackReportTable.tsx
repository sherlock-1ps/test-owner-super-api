...
const CashBackReportTable = ({ activeCard }: { activeCard: number }) => {
  // States
  ...
  const columns = useMemo<ColumnDef<InvoiceTypeWithAction, any>[]>(
    () => [
      ...
      columnHelper.accessor('total', {
        header: 'Total bet',
        cell: ({ row }) => <Typography color={'success.main'}>{`$ ${row.original.total} บาท`}</Typography>
      }),
      columnHelper.accessor('issuedDate', {
        header: 'Loss Amount',
        cell: ({ row }) => <Typography color={'error.main'}>$ {row.original.issuedDate}</Typography>
      }),
      columnHelper.accessor('cashback', {
        header: 'Cashback',
        cell: ({ row }) => {
          return <Typography color={'warning.main'}>$ {row.original.cashback}</Typography>
        }
      })
    ],
    // eslint-disable-next-line react-hooks/exhaustive-deps
...
export default CashBackReportTable
