  ...
  const columns = useMemo<ColumnDef<InvoiceTypeWithAction, any>[]>(
    () => [
      columnHelper.accessor('id', {
        ...
        cell: ({ row }) => (
          <Typography
            ...
            // color='primary'
            variant='h6'
          >{`XPB#${row.original.id}`}</Typography>
        )
      }),
      ...
      columnHelper.accessor('total', {
        ...
        cell: ({ row }) => <Typography>{`$ ${row.original.total} บาท`}</Typography>
      }),
      ...
      columnHelper.accessor('balance', {
        header: 'Total Betting',
        cell: ({ row }) => {
          return <Typography color='text.primary'>{row.original.balance}</Typography>
        }
      })
    ],
    // eslint-disable-next-line react-hooks/exhaustive-deps
