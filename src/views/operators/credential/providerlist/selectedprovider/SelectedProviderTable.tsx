...
// Column Definitions
const columnHelper = createColumnHelper<GameType>()
...
const SelectedProviderTable = ({ data = [], page, pageSize, setPage, setPageSize, handleRefetchSearch }: any) => {
  ...
  const { mutate: toggleGameProviderStatus, isPending: pendingUpdate } =
    useUpdateStatusGameProviderMutationOption(handleRefetchSearch)
  ...
  const columns = useMemo<ColumnDef<GameType, any>[]>(
    () => [
      ...
      columnHelper.accessor('create_at', {
        ...
        cell: ({ row }) => (
          <div className='flex flex-col'>
            <Typography variant='h6'>{row.original.create_at}</Typography>
          </div>
        )
      }),
  ...
  const table = useReactTable<GameType>({
    data: data.list,
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
...
export default SelectedProviderTable
