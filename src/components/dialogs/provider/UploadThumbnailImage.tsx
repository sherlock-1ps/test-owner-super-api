...
const UploadThumbnailImage = ({ setFileImg }: any) => {
  // States
  ...
  const renderFilePreview = (file: File) => {
    return <img width={138} height={200} alt={file.name} src={URL.createObjectURL(file)} />
  }
  ...
  const fileList = files.map(file => (
    <ListItem key={file.name} className='pis-4 plb-3 w-full flex items-center justify-center'>
        ...
        {files.length ? (
          ...
          <div {...getRootProps({ className: 'dropzone' })}>
            ...
            <div className='flex items-center flex-col gap-2 text-center'>
              ...
              <Typography className='font-bold'>Upload New Thumbnail</Typography>
              <Typography color='text.disabled'>WebP format, Up to 1MB, 320X460px</Typography>
            </div>
          </div>
        )}
      </>
